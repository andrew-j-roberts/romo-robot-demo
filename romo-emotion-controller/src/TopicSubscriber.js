  ...
  subscriber.connect = function() {
    ...
    subscriber.session.on(solace.SessionEventCode.MESSAGE, function(message) {
      ...
      if (jsonData.FaceDetails != undefined) {
        ...
        let messageText = `[{ "expression":"${romoExpression}", "emotion":"${romoEmotion}", "highestEmotion":"${highestEmotion.toLowerCase()}", "highestConfidence":"${highestConfidence}", "source":"${source}", "Sharpness":"${highestSharpness}", "Brightness":"${highestBrightness}", "image":"${imagekey}}]`;
